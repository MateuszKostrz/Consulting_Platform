...
function loadSearchData() {
  ...
  console.log('Loading search data from:', assetsPath + 'json/' + searchJson);
  
  // First try to load from the assetsPath
  fetch(assetsPath + 'json/' + searchJson)
    ...
    .catch(error => {
      console.error('Error loading JSON from primary path:', error);
      
      // If that fails, try the static path
      const staticPath = '/static/website/json/';
      console.log('Trying fallback path:', staticPath + searchJson);
      
      fetch(staticPath + searchJson)
        .then(response => {
          if (!response.ok) throw new Error('Failed to fetch data from fallback path');
          return response.json();
        })
        .then(json => {
          data = json;
          initializeAutocomplete();
          console.log('Search data loaded successfully from fallback path');
        })
        .catch(err => console.error('Error loading JSON from fallback path:', err));
    });
}
...
// Add click handler for search toggler
document.addEventListener('DOMContentLoaded', function() {
  const searchTogglers = document.querySelectorAll('.search-toggler');
  searchTogglers.forEach(toggler => {
    toggler.addEventListener('click', function() {
      // Try to find and click the Algolia search button
      const searchButton = document.querySelector('.aa-DetachedSearchButton');
      if (searchButton) {
        searchButton.click();
      } else {
        console.error('Search button not found - autocomplete may not be initialized');
        // Try to reload search data if button not found
        loadSearchData();
      }
    });
  });
