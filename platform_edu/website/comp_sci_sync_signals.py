"""
Django Signals for Computer Science SL to HL Auto-Sync

These signals automatically sync Computer Science SL questions to Computer Science HL when:
...
from .models import Comp_Sci_SL_Questionbank, Comp_Sci_HL_Questionbank
import hashlib
import logging
...
def find_corresponding_hl_question(sl_question):
    ...
    try:
        ...
        if sl_question.hl_question_id:
            try:
                hl_question = Comp_Sci_HL_Questionbank.objects.get(id=sl_question.hl_question_id)
                return hl_question
            except Comp_Sci_HL_Questionbank.DoesNotExist:
        ...
        hl_question = Comp_Sci_HL_Questionbank.objects.filter(
        ...
        hl_questions = Comp_Sci_HL_Questionbank.objects.filter(
            question=sl_question.question
        )
...
@receiver(post_save, sender=Comp_Sci_SL_Questionbank)
def sync_sl_to_hl_on_save(sender, instance, created, **kwargs):
    ...
    if not instance.sync_to_hl:
        if instance.hl_question_id:
            try:
                Comp_Sci_HL_Questionbank.objects.filter(id=instance.hl_question_id).delete()
                Comp_Sci_SL_Questionbank.objects.filter(id=instance.id).update(hl_question_id=None)
    ...
    sl_chapters = dict(Comp_Sci_SL_Questionbank.CHAPTERS)
    hl_chapters = dict(Comp_Sci_HL_Questionbank.CHAPTERS)
        ...
        if created:
            ...
            if corresponding_hl:
                ...
                if instance.hl_question_id != corresponding_hl.id:
                    ...
                    Comp_Sci_SL_Questionbank.objects.filter(id=instance.id).update(hl_question_id=corresponding_hl.id)
                
                logger.info(f"Updated HL question {corresponding_hl.id} from SL question {instance.id}")
            else:
                # Create new HL question
                new_hl_question = Comp_Sci_HL_Questionbank.objects.create(
                ...
                Comp_Sci_SL_Questionbank.objects.filter(id=instance.id).update(hl_question_id=new_hl_question.id)
...
@receiver(post_delete, sender=Comp_Sci_SL_Questionbank)
