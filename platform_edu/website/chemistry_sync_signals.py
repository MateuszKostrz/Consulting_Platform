"""
Django Signals for Chemistry SL to HL Auto-Sync

These signals automatically sync Chemistry SL questions to Chemistry HL when:
...
from .models import Chemistry_SL_Questionbank, Chemistry_HL_Questionbank
import hashlib
import logging
...
def find_corresponding_hl_question(sl_question):
    ...
    try:
        ...
        if sl_question.hl_question_id:
            try:
                hl_question = Chemistry_HL_Questionbank.objects.get(id=sl_question.hl_question_id)
                return hl_question
            except Chemistry_HL_Questionbank.DoesNotExist:
                # The linked HL question was deleted, clear the link
                sl_question.hl_question_id = None
                sl_question.save()
        
        # Try to find by exact question text match
        hl_question = Chemistry_HL_Questionbank.objects.filter(
        ...
        hl_questions = Chemistry_HL_Questionbank.objects.filter(chapter=sl_question.chapter)
...
@receiver(post_save, sender=Chemistry_SL_Questionbank)
def sync_sl_to_hl_on_save(sender, instance, created, **kwargs):
    ...
    sl_chapters = dict(Chemistry_SL_Questionbank.CHAPTERS)
    hl_chapters = dict(Chemistry_HL_Questionbank.CHAPTERS)
        ...
        if created:
            ...
            if corresponding_hl:
                ...
                if instance.hl_question_id != corresponding_hl.id:
                    ...
                    Chemistry_SL_Questionbank.objects.filter(id=instance.id).update(hl_question_id=corresponding_hl.id)
                
                logger.info(f"Updated HL question {corresponding_hl.id} from SL question {instance.id}")
            else:
                # Create new HL question
                new_hl_question = Chemistry_HL_Questionbank.objects.create(
                ...
                Chemistry_SL_Questionbank.objects.filter(id=instance.id).update(hl_question_id=new_hl_question.id)
...
@receiver(post_delete, sender=Chemistry_SL_Questionbank)
def sync_sl_to_hl_on_delete(sender, instance, **kwargs):
    ...
    try:
        ...
        hl_questions = Chemistry_HL_Questionbank.objects.filter(
            chapter=instance.chapter,
            question=instance.question
        )
