"""
Django Signals for Physics SL to HL Auto-Sync

These signals automatically sync Physics SL questions to Physics HL when:
...
from .models import Physics_SL_Questionbank, Physics_HL_Questionbank
import hashlib
import logging
...
def find_corresponding_hl_question(sl_question):
    ...
    try:
        ...
        if sl_question.hl_question_id:
            try:
                hl_question = Physics_HL_Questionbank.objects.get(id=sl_question.hl_question_id)
                return hl_question
            except Physics_HL_Questionbank.DoesNotExist:
...
@receiver(post_save, sender=Physics_SL_Questionbank)
def sync_sl_to_hl_on_save(sender, instance, created, **kwargs):
    ...
    sl_chapters = dict(Physics_SL_Questionbank.CHAPTERS)
    hl_chapters = dict(Physics_HL_Questionbank.CHAPTERS)
        ...
        if created:
            ...
            if corresponding_hl:
                ...
                if instance.hl_question_id != corresponding_hl.id:
                    ...
                    Physics_SL_Questionbank.objects.filter(id=instance.id).update(hl_question_id=corresponding_hl.id)
                
                logger.info(f"Updated HL question {corresponding_hl.id} from SL question {instance.id}")
            else:
                # Create new HL question
                new_hl_question = Physics_HL_Questionbank.objects.create(
                ...
                Physics_SL_Questionbank.objects.filter(id=instance.id).update(hl_question_id=new_hl_question.id)
...
@receiver(post_delete, sender=Physics_SL_Questionbank)
def sync_sl_to_hl_on_delete(sender, instance, **kwargs):
    ...
    try:
        ...
        hl_questions = Physics_HL_Questionbank.objects.filter(
            chapter=instance.chapter,
            question=instance.question
        )
