"""
Django Signals for Math AA SL to HL Auto-Sync

These signals automatically sync Math AA SL questions to Math AA HL when:
...
from .models import Math_AA_SL_Questionbank, Math_AA_HL_Questionbank
import hashlib
import logging
...
def find_corresponding_hl_question(sl_question):
    ...
    try:
        ...
        if sl_question.hl_question_id:
            try:
                hl_question = Math_AA_HL_Questionbank.objects.get(id=sl_question.hl_question_id)
                return hl_question
            except Math_AA_HL_Questionbank.DoesNotExist:
                # The linked HL question was deleted, clear the link
                sl_question.hl_question_id = None
                sl_question.save()
        
        # Try to find by exact question text match
        hl_question = Math_AA_HL_Questionbank.objects.filter(
        ...
        hl_questions = Math_AA_HL_Questionbank.objects.filter(chapter=sl_question.chapter)
...
@receiver(post_save, sender=Math_AA_SL_Questionbank)
def sync_sl_to_hl_on_save(sender, instance, created, **kwargs):
    ...
    if not instance.sync_to_hl:
        if instance.hl_question_id:
            try:
                Math_AA_HL_Questionbank.objects.filter(id=instance.hl_question_id).delete()
                Math_AA_SL_Questionbank.objects.filter(id=instance.id).update(hl_question_id=None)
        ...
        if created:
            ...
            if corresponding_hl:
                ...
                if instance.hl_question_id != corresponding_hl.id:
                    ...
                    Math_AA_SL_Questionbank.objects.filter(id=instance.id).update(hl_question_id=corresponding_hl.id)
                
                logger.info(f"Updated HL question {corresponding_hl.id} from SL question {instance.id}")
            else:
                # Create new HL question
                new_hl_question = Math_AA_HL_Questionbank.objects.create(
                ...
                Math_AA_SL_Questionbank.objects.filter(id=instance.id).update(hl_question_id=new_hl_question.id)
...
@receiver(post_delete, sender=Math_AA_SL_Questionbank)
