"""
Django Management Command: Fix Math AI SL sync status
...
from website.models import Math_AI_SL_Questionbank, Math_AI_HL_Questionbank
import hashlib

class Command(BaseCommand):
    help = 'Fix Math AI SL sync status to reflect actual sync with HL'
    ...
    def handle(self, *args, **options):
        if options['analyze']:
            ...
            self.stdout.write('  python manage.py fix_math_ai_sync_status --analyze    # Show analysis')
            self.stdout.write('  python manage.py fix_math_ai_sync_status --fix       # Fix sync status')
    ...
    def find_matching_hl_question(self, sl_question):
        ...
        hl_questions = Math_AI_HL_Questionbank.objects.filter(chapter=sl_question.chapter)
    ...
    def analyze_sync_status(self):
        ...
        self.stdout.write(self.style.SUCCESS('📊 Math AI SL/HL Sync Status Analysis'))
        self.stdout.write('=' * 60)
        
        # Get all SL questions
        sl_questions = Math_AI_SL_Questionbank.objects.all().order_by('chapter', 'id')
    ...
    def fix_sync_status(self):
        ...
        self.stdout.write(self.style.SUCCESS('🔧 Fixing Math AI SL/HL Sync Status'))
        self.stdout.write('=' * 50)
        
        fixed_count = 0
        
        with transaction.atomic():
            sl_questions = Math_AI_SL_Questionbank.objects.all()
