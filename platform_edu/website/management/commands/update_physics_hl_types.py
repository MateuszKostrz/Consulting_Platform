"""
Management command to update Physics HL question types for HL-only chapters:
1. First 3 Paper 1A questions per chapter = 'free'
2. At least 50% of all questions = 'premium' (including all Paper 2 except 1)
3. Remaining questions (including 1 Paper 2) = 'registered'

Only processes chapters that are unique to HL (not in SL).
"""

from django.core.management.base import BaseCommand
from website.models import Physics_HL_Questionbank
import random


class Command(BaseCommand):
    help = 'Updates Physics HL question types for HL-only chapters according to specified distribution rules'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('Starting Physics HL question type update...'))
        
        # Define SL chapters (these will be EXCLUDED)
        sl_chapters = [
            'kinematics',
            'forces_and_momentum',
            'work_energy_power',
        ...
        # HL-only chapters that we WILL process
        hl_only_chapters = [
        ...
        self.stdout.write(f'\nHL-only chapters to process: {", ".join(hl_only_chapters)}')
        self.stdout.write(f'Excluding SL chapters: {", ".join(sl_chapters)}\n')
        
        # Get all unique chapters in the database
        all_chapters = Physics_HL_Questionbank.objects.values_list('chapter', flat=True).distinct()
        
        # Filter to only HL-only chapters
        chapters_to_process = [ch for ch in all_chapters if ch in hl_only_chapters]
        
        if not chapters_to_process:
            self.stdout.write(self.style.WARNING('No HL-only chapters found in database!'))
            return
        
        self.stdout.write(f'Found {len(chapters_to_process)} HL-only chapters in database\n')
        
        total_updated = 0
        summary = []
        
        for chapter in chapters_to_process:
            self.stdout.write(f'Processing chapter: {chapter}')
            
            # Get all questions for this chapter
            all_questions = list(Physics_HL_Questionbank.objects.filter(chapter=chapter))
            ...
            # STEP 2: Calculate how many questions should be premium (at least 50% of total)
            # Use ceiling division to ensure we get AT LEAST 50%
            target_premium_count = (total_questions + 1) // 2  # This rounds up
            ...
            premium_percentage = (count_premium / total_questions * 100) if total_questions > 0 else 0
            
            # Verify we have at least 50% premium
            if premium_percentage < 50.0:
                self.stdout.write(self.style.WARNING(f'  ⚠️  Warning: Premium percentage ({premium_percentage:.1f}%) is below 50%!'))
            
            self.stdout.write(self.style.SUCCESS(f'  Updated {chapter}:'))
            self.stdout.write(f'    Free: {count_free} questions')
            self.stdout.write(f'    Premium: {count_premium} questions ({premium_percentage:.1f}%)')
            ...
            summary.append({
                ...
                'premium_pct': premium_percentage
            })
        
        # Display final summary
        self.stdout.write('='*70)
        self.stdout.write(self.style.SUCCESS('SUMMARY - Physics HL (HL-only chapters)'))
        self.stdout.write('='*70)
        ...
        if summary:
            ...
            # Check if we met the 50% premium requirement
            if (total_premium / grand_total * 100) >= 50.0:
                self.stdout.write(self.style.SUCCESS(f'\n✓ Premium requirement met: {total_premium/grand_total*100:.1f}% >= 50%'))
            else:
                self.stdout.write(self.style.WARNING(f'\n✗ Premium requirement NOT met: {total_premium/grand_total*100:.1f}% < 50%'))
            
            self.stdout.write('\n' + self.style.SUCCESS('Physics HL question types updated successfully!'))
        else:
