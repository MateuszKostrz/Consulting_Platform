...
class Command(BaseCommand):
    help = 'Fix nested <p class="answers-ms"> tags in all Math AI HL questions'

    def handle(self, *args, **kwargs):
        self.stdout.write(f"\n{'='*60}")
        self.stdout.write("Fixing all Math AI HL questions")
        self.stdout.write(f"{'='*60}\n")
        
        # Get all questions from Math AI HL
        questions = Math_AI_HL_Questionbank.objects.all()
        
        self.stdout.write(f"Total questions in Math AI HL: {questions.count()}")
        ...
        if fixed_count > 0:
            self.stdout.write(self.style.SUCCESS(
                f"Successfully fixed {fixed_count} questions in Math AI HL"
            ))
            ...
            self.stdout.write(self.style.SUCCESS(
                "No questions with nested tags found in Math AI HL"
            ))
        self.stdout.write(f"{'='*60}\n")
