"""
Django Management Command: Fix Biology SL sync status
...
class Command(BaseCommand):
    help = 'Fix Biology SL sync status to reflect actual sync with HL'
    ...
    def handle(self, *args, **options):
        if options['analyze']:
            ...
            self.stdout.write('  python manage.py fix_biology_sync_status --analyze    # Show analysis')
            self.stdout.write('  python manage.py fix_biology_sync_status --fix       # Fix sync status')
    ...
    def analyze_sync_status(self):
        ...
        self.stdout.write(self.style.SUCCESS('🔍 Biology SL/HL Sync Status Analysis'))
        self.stdout.write('=' * 60)
        
        # Get all SL questions
        sl_questions = Biology_SL_Questionbank.objects.all().order_by('chapter', 'id')
    ...
    def fix_sync_status(self):
        ...
        self.stdout.write(self.style.SUCCESS('🔧 Fixing Biology SL/HL Sync Status'))
        self.stdout.write('=' * 50)
        
        fixed_count = 0
        
        with transaction.atomic():
            sl_questions = Biology_SL_Questionbank.objects.all()
